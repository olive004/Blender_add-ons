...
# GLOBAL VAR'S
filepath_vector = '/Users/oliviagallup/Desktop/Laser_Cutz/dragon.svg'    # Pathname of svg to import
# Color
mat_color =  [0.158577, 0.027596, 0.066986]      # RGB color scheme of obj
engraved_mat_color = [1.000000, 0.166923, 0.000000]
bg_color = [0.068651, 0.000544, 0.000000]       # RGB color of horizon (in 'world' settings)
zenith_color = [0.005403, 0.003859, 0.335936]   # Gradient bg: TURN OFF BY COMMENTING OUT 'use_blend_sky' under RENDER
emit = 0.7         # Fluorescence of obj
emit_engraved = 1.2
hardness = 13      # The light bouncing inside the object depends on hardness too
alpha = 0.324925      # Main (alpha) transparency of obj
extrusion_length = -0.356
extrusion_length_engraved = extrusion_length + 0.06
frame_count = 36
render_mode = 'SKY'     # Background color can be either 'SKY' or 'TRANSPARENT'
output_filepath  = '/Users/oliviagallup/Desktop/Blendr/Animations/dragon'   # Directory to output the png's to
...
bpy.data.materials[obj_mat].specular_hardness = hardness        # hardness
...
bpy.data.materials[engraved_mat.name].use_transparency = True
bpy.data.materials[engraved_mat.name].transparency_method = 'RAYTRACE'
bpy.data.materials[engraved_mat.name].alpha = alpha
bpy.data.materials[engraved_mat.name].emit = emit_engraved
bpy.data.materials[engraved_mat.name].specular_intensity = 0.6
...
bpy.ops.mesh.extrude_region_move(MESH_OT_extrude_region={"mirror":False}, TRANSFORM_OT_translate={"value":(0, 0, extrusion_length_engraved), "constraint_axis":(False, False, True), "constraint_orientation":'NORMAL', "mirror":False, "proportional":'DISABLED', "proportional_edit_falloff":'SMOOTH', "proportional_size":1, "snap":False, "snap_target":'CLOSEST', "snap_point":(0, 0, 0), "snap_align":False, "snap_normal":(0, 0, 0), "gpencil_strokes":False, "texture_space":False, "remove_on_cancel":False, "release_confirm":False})     # phat funciton to extrude in z direction by an amount
...
bpy.data.worlds["World"].zenith_color = zenith_color
